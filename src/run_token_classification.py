...
from metric_11 import Metric
from models import SpellBert, SpellBertArch3MLM, SpellBertArch3,SpellBertForTokenClassification

import pickle

#add by cui
from metric_token_classification import compute_prf,token_evaluation_batch
...
MODEL_CLASSES = {
    ...
    'bert-arch3': (BertConfig, SpellBertArch3, BertTokenizer),
    'bert-arch3-mlm': (BertConfig, SpellBertArch3MLM, BertTokenizer),
    'bert-token-classification':(BertConfig, SpellBertForTokenClassification, BertTokenizer),
}
...
def make_features(args, examples, tokenizer, batch_processor):
    ...
    batch = {}
    for t in ['id', 'src', 'tgt', 'tokens_size', 'lengths', 'src_idx', 'tgt_idx', 'masks', 'loss_masks','token_labels']:
        batch[t] = []
    for item in examples:
        for t in item:
            if t == 'src_idx' or t == 'tgt_idx' :
                ...
                seq = item[t][:max_length]
                padding_length = max_length - len(seq)
                batch['token_labels'].append(seq+ ([-100] * padding_length))
    ...
    batch['token_labels'] = torch.tensor(batch['token_labels'],dtype=torch.long)

    batch = batch_processor(batch, tokenizer)
    return batch


def data_helper(args, dataset, tokenizer, batch_processor):
    if args.do_train:
...
def train(args, model, tokenizer, batch_processor):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(data_helper(args, train_dataset, tokenizer, batch_processor)):
            model.train()
            for t in batch:
                if t not in ['id', 'src', 'tgt', 'lengths', 'tokens_size']:
...
def evaluate(args, model, tokenizer, batch_processor, prefix=""):
    ...
    for batch in data_helper(args, eval_dataset, tokenizer, batch_processor):
        model.eval()
        ...
        for sentence in predict_score:
            ...
            for word in sentence:
                if word[0] / word[1] > 100 or word[1] / word[0] > 100:
                    predict.append(int(torch.argmax(word)))
                else:
                    predict.append(1)
            predictions.append(predict)
        preds = torch.tensor(predictions)
        TP,FP,FN = token_evaluation_batch(preds,token_labels)
        TP += TP
        FP += FP
        FN += FN
    results = compute_prf(TP,FP,FN)
    print(results)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_dir", default=None, type=str, required=True,
                        help="The output_add_sen_loss_0.5 directory where the model predictions and checkpoints will be written.")
    ...
    parser.add_argument('--overwrite_output_dir', action='store_true',
                        help="Overwrite the content of the output_add_sen_loss_0.5 directory")
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        # Create output_add_sen_loss_0.5 directory if needed
