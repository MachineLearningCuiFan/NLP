...
from models import SpellBert, SpellBertArch3MLM, SpellBertArch3,SpellBertArch3AddSenLoss
...
MODEL_CLASSES = {
    ...
    'bert-arch3-add-sen-loss':(BertConfig, SpellBertArch3AddSenLoss, BertTokenizer)
}
...
def make_features(args, examples, tokenizer, batch_processor):
    ...
    batch = {}
    for t in ['id', 'src', 'tgt', 'tokens_size', 'lengths', 'src_idx', 'tgt_idx', 'masks', 'loss_masks','sen_labels']:
        batch[t] = []
    for item in examples:
        for t in item:
    ...
    batch['sen_labels'] = torch.tensor(batch['sen_labels'], dtype=torch.long)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_steps', type=int, default=18000,
