...
from models import SpellBertArch3,SpellBert,SpellBertArch3MLM


MODEL_CLASSES = {
    'bert-arch3': SpellBertArch3,
    'bert':SpellBert,
}
...
def test(ckpt_dir, data_dir, ckpt_num, testset_year, output_dir, device):
    ...
    elif testset_year == 16:
        test_picke_path = os.path.join(data_dir, 'test.renmin_200.pkl')
        label_path = os.path.join(data_dir, 'test.renmin_200.lbl.tsv')
    elif testset_year == 17:
        test_picke_path = os.path.join(data_dir, 'test.novel_100.pkl')
        label_path = os.path.join(data_dir, 'test.novel_100.lbl.tsv')
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--testset_year', type=int, required=True, choices=[15, 14, 13,16,17])
